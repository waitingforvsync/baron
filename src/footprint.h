...
// it calls - the interprocedural core (the spec's Touch(R)). Anything a caller holds live across a
// JSR collides with the callee's ENTIRE footprint, because the call clobbers all of it. unknown_call
// means a JSR with an untrackable target was reached, so the footprint is incomplete - the caller
// must refuse rather than under-approximate.
//
// recursive means the call graph reached from this entry contains a cycle. Recursion alone is NOT
// fatal: a value merely read or read-modified across it (a shared accumulator, DEC/INC) rides safely
// on one static byte. What cannot is a value the cycle FRESHLY writes (a write-only def, in killed)
// and carries live across the call - that wants a distinct byte per level. So the caller refuses
// only when a live-across variable is also in killed of a recursive footprint.

...
{
    dfs_name d = {.dir = '$', .name = filename};
    if (filename.len >= 2 && filename.data[1] == '.') {   // "X.File": a directory specifier
        d.dir = filename.data[0];
        d.name = rc_str_skip(filename, 2);
    }
    if (!dfs_char_ok(d.dir) || d.name.len == 0 || d.name.len > 7) {
        return (dfs_name) {0};
    }
    for (uint32_t i = 0; i < d.name.len; i++) {
        if (!dfs_char_ok(d.name.data[i])) {
            return (dfs_name) {0};
        }
    }
    d.ok = true;
...
disc_ssd_result disc_ssd_make(const output_spec *spec, rc_arena *arena)
{
    ...
    // entry order. starts[] remembers where each landed for the catalogue below.
    dfs_name *names  = n != 0 ? rc_arena_alloc_type(arena, dfs_name, n) : NULL;
    uint32_t *starts = n != 0 ? rc_arena_alloc_type(arena, uint32_t, n) : NULL;
    uint32_t next = ssd_first_sector;
    for (uint32_t i = 0; i < n; i++) {
        output_entry e = rc_view_output_entry_get(spec->entries, i);
        names[i] = dfs_name_parse(e.filename);
        if (!names[i].ok) {
        ...
        for (uint32_t j = 0; j < i; j++) {
            if (dfs_name_is_equal(names[i], names[j])) {
    ...
    for (uint32_t slot = 0; slot < n; slot++) {
        ...
        for (uint32_t c = 0; c < 7; c++) {
            rc_array_bytes_set(&img, name_off + c, c < names[i].name.len ? (uint8_t) names[i].name.data[c] : ' ');
        }
        rc_array_bytes_set(&img, name_off + 7, (uint8_t) names[i].dir);
        ...
        rc_array_bytes_set(&img, off + 6, (uint8_t) (((starts[i] >> 8) & 3)
                                                   | (((load >> 16) & 3) << 2)
                                                   | (((len >> 16) & 3) << 4)
                                                   | (((exec >> 16) & 3) << 6)));
        rc_array_bytes_set(&img, off + 7, (uint8_t) (starts[i] & 0xFF));
    }

    return (disc_ssd_result) {.image = img};
}
...
RC_TEST(disc_ssd, catalogue_layout_and_truncation)
{
    ...
    RC_CHECK((uint32_t) rc_array_bytes_get(&r.image, 0), ==, (uint32_t) 'M');
    RC_CHECK((uint32_t) rc_array_bytes_get(&r.image, 5), ==, (uint32_t) 'c');
    RC_CHECK((uint32_t) rc_array_bytes_get(&r.image, 6), ==, 0u);
    RC_CHECK((uint32_t) rc_array_bytes_get(&r.image, 256), ==, 0u);
    RC_CHECK((uint32_t) rc_array_bytes_get(&r.image, 256 + 4), ==, 0x42u);
    RC_CHECK((uint32_t) rc_array_bytes_get(&r.image, 256 + 5), ==, 16u);
    RC_CHECK((uint32_t) rc_array_bytes_get(&r.image, 256 + 6), ==, 0x33u);
    RC_CHECK((uint32_t) rc_array_bytes_get(&r.image, 256 + 7), ==, 0x20u);
    ...
    RC_CHECK((uint32_t) rc_array_bytes_get(&r.image, 8), ==, (uint32_t) 'd');
    RC_CHECK((uint32_t) rc_array_bytes_get(&r.image, 8 + 4), ==, (uint32_t) ' ');   // space-padded to 7
    RC_CHECK((uint32_t) rc_array_bytes_get(&r.image, 8 + 7), ==, (uint32_t) 'X');
    RC_CHECK((uint32_t) rc_array_bytes_get(&r.image, 256 + 8 + 0), ==, 0x00u);
    RC_CHECK((uint32_t) rc_array_bytes_get(&r.image, 256 + 8 + 1), ==, 0x20u);
    RC_CHECK((uint32_t) rc_array_bytes_get(&r.image, 256 + 8 + 4), ==, 0x2Cu);
    RC_CHECK((uint32_t) rc_array_bytes_get(&r.image, 256 + 8 + 5), ==, 0x01u);
    RC_CHECK((uint32_t) rc_array_bytes_get(&r.image, 256 + 8 + 6), ==, 0x00u);
    RC_CHECK((uint32_t) rc_array_bytes_get(&r.image, 256 + 8 + 7), ==, 3u);
    ...
    RC_CHECK((uint32_t) rc_array_bytes_get(&r.image, 16), ==, (uint32_t) 'b');
    RC_CHECK((uint32_t) rc_array_bytes_get(&r.image, 16 + 7), ==, (uint32_t) '$');
    RC_CHECK((uint32_t) rc_array_bytes_get(&r.image, 256 + 16 + 0), ==, 0x00u);
    RC_CHECK((uint32_t) rc_array_bytes_get(&r.image, 256 + 16 + 1), ==, 0x19u);
    RC_CHECK((uint32_t) rc_array_bytes_get(&r.image, 256 + 16 + 2), ==, 0x03u);
    RC_CHECK((uint32_t) rc_array_bytes_get(&r.image, 256 + 16 + 3), ==, 0x19u);
    RC_CHECK((uint32_t) rc_array_bytes_get(&r.image, 256 + 16 + 4), ==, 0x03u);
    RC_CHECK((uint32_t) rc_array_bytes_get(&r.image, 256 + 16 + 5), ==, 0x00u);
    RC_CHECK((uint32_t) rc_array_bytes_get(&r.image, 256 + 16 + 6), ==, 0xCCu);
    RC_CHECK((uint32_t) rc_array_bytes_get(&r.image, 256 + 16 + 7), ==, 2u);

    // The file bytes themselves: boot at sector 2 (zero-padded to its end), data at sector 3.
    RC_CHECK((uint32_t) rc_array_bytes_get(&r.image, 512), ==, 0xA9u);
    RC_CHECK((uint32_t) rc_array_bytes_get(&r.image, 514), ==, 0x60u);
    RC_CHECK((uint32_t) rc_array_bytes_get(&r.image, 515), ==, 0u);
    RC_CHECK((uint32_t) rc_array_bytes_get(&r.image, 768), ==, 0x11u);
    RC_CHECK((uint32_t) rc_array_bytes_get(&r.image, 768 + 299), ==, 0x99u);
    RC_CHECK((uint32_t) rc_array_bytes_get(&r.image, 768 + 300), ==, 0u);
...
RC_TEST(disc_ssd, empty_disc)
{
    ...
    RC_CHECK((uint32_t) rc_array_bytes_get(&r.image, 256 + 5), ==, 0u);
    RC_CHECK((uint32_t) rc_array_bytes_get(&r.image, 256 + 6), ==, 3u);
    RC_CHECK((uint32_t) rc_array_bytes_get(&r.image, 256 + 7), ==, 0x20u);
    rc_arena_deinit(&arena);
}

...
enum {
    ...
    zeropage_entries_reserve = 8,   // entry markers are one per externally-visible routine; grows if exceeded
};
...
void zeropage_init(zeropage *zp, rc_arena *permanent)
{
    ...
    zp->entries  = rc_array_zp_entry_make(zeropage_entries_reserve, permanent);
    zp->enabled  = false;
}
...
void zeropage_reset(zeropage *zp)
{
    ...
    rc_array_zp_entry_resize(&zp->entries, 0, zp->arena);
    zp->enabled = false;
}
...
{
    RC_ASSERT(zp != NULL);
    uint32_t index = zp->entries.num;
    rc_array_zp_entry_push(&zp->entries, e, zp->arena);
    return index;
}
...
RC_TEST(zeropage, entry_registry)
{
    rc_arena arena = rc_arena_make_default();
    zeropage zp;
    zeropage_init(&zp, &arena);

    RC_CHECK(zeropage_entries(&zp).num, ==, 0u);

    // One sync entry and one interrupt handler; the record is a plain position + flavour.
    uint32_t i0 = zeropage_add_entry(&zp, (zp_entry) {.section = 0, .pc = 0x2000, .interrupt = false,
                                                      .at = (cursor) {.source = 0, .pos = 5}});
    uint32_t i1 = zeropage_add_entry(&zp, (zp_entry) {.section = 1, .pc = 0x8003, .interrupt = true,
                                                      .at = (cursor) {.source = 0, .pos = 50}});
    RC_CHECK(i0, ==, 0u);
    RC_CHECK(i1, ==, 1u);

    rc_view_zp_entry entries = zeropage_entries(&zp);
    RC_CHECK(entries.num, ==, 2u);
    zp_entry e0 = rc_view_zp_entry_get(entries, 0);
    RC_CHECK(e0.section, ==, 0u);
    RC_CHECK(e0.pc, ==, 0x2000u);
    RC_CHECK_FALSE(e0.interrupt);
    RC_CHECK(e0.at.pos, ==, 5u);
    zp_entry e1 = rc_view_zp_entry_get(entries, 1);
    RC_CHECK(e1.section, ==, 1u);
    RC_CHECK_TRUE(e1.interrupt);
...
RC_TEST(zeropage, reset_clears_for_next_pass)
{
    ...
    RC_CHECK(zeropage_entries(&zp).num, ==, 1u);

    // A new pass starts from an empty, dormant set with no vars, insns, labels or entries (backing kept).
    zeropage_reset(&zp);
    RC_CHECK_FALSE(zeropage_is_enabled(&zp));
    RC_CHECK(zeropage_reserved_count(&zp), ==, 0u);
    RC_CHECK_FALSE(zeropage_is_reserved(&zp, 0x70));

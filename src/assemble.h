...
// What an assemble produced: a read-only, position-independent snapshot. Every field borrows from the
// arenas that were passed in, so the result is valid until the next assemble on those same arenas (or
// until the arenas are freed). passes == 0 means failure - sections then holds only an empty default
// section, scopes is an empty tree, and diagnostics carries the errors.
//
// sections is the whole object-code list (index 0 is the default section; each carries its pc + code) -
// iterate it directly, or use baron_result_code for the default section's bytes alone. scopes is a
// read-only view of the resolved scope tree (its backing outlives the internal machine): look a single
// symbol up by full dotted path with baron_result_symbol / scopes_view_get_symbol, or harvest the whole
// spellable table on demand with scopes_view_flatten (which needs an arena to build the paths into).
//
// sources is every source the assemble touched (the root plus anything INCLUDEd), each a name + full
// text: a diagnostic's cursor holds a source INDEX and a byte OFFSET, and this view is what turns them
// back into a file name and a line/column. Range-check the index before resolving - the one diagnostic
// with nowhere real to point (an unreadable root file) carries a cursor no source was registered for.
//
// channels is the PRINT output, one text stream per channel (PRINT #n, ...; no #n means 0). Channel 0
// also carries the assembly listing when the desc asked for it (baron_desc.verbose): the listing pass
// runs after zero-page allocation and both write the same buffer, so PRINT output lands interleaved
// between listing lines - and the returned sections come from that pass too, so what the listing shows
// IS the output (without -v, the sections are the settling pass's, patched by the allocator - the same
// bytes either way, and PRINT writes on that pass instead).
//
// Lifetimes differ by field: sections and channels borrow from the per_pass arena and are superseded by
// the next assemble on the same arenas; diagnostics, scopes and sources are permanent-backed, so earlier
